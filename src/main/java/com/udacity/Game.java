...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        //grid[i][j]: i - columns, j - rows

        for(int i=0; i<grid.length; i++){
            if(grid[i][0]==grid[i][1] && grid[i][1]==grid[i][2]) {//check cols
                result = winMessage(grid[i][0]);
            }else if(grid[0][i]==grid[1][i] && grid[1][i]==grid[2][i]){//check rows
                result = winMessage(grid[0][i]);
            }else if(grid[0][0]==grid[1][1] && grid[1][1]==grid[2][2]){//check diagonal left to right
                result = winMessage(grid[0][0]);
            }else if(grid[0][2]==grid[1][1] && grid[1][1]==grid[2][0]) {//check diagonal right to left
                result = winMessage(grid[0][2]);
            }else{
                int score=0;
                for(int k=0; k<grid.length; k++){
                    for(int j=0; j<grid.length; j++){
                        if(grid[k][j]!='-'){
                            score++;
                            if(score==9) result = "Tie";
                        }
                    }
                }
            }
        }
        //result returns "X wins", "Y wins", "Tie" or "None"
        return result;
    }

    public String winMessage(char input){
        String message = "None";
        if(input!='-'){
            if(input=='x'){
                message = "X wins";
